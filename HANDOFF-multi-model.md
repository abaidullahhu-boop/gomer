# Handoff — multi-model support (Gaspo)
...
The workspace's Anthropic key is **out of credits**, so every AI run fails. Gaspo was
...
inventing prices → mis-billing. `supportsTools` defaults to **false** — Gaspo is 100%
...
- No full Gaspo run has completed on a gateway model — only the raw provider call below.
...
| `veoaifree-web` | 6 | video models — irrelevant to Gaspo |
