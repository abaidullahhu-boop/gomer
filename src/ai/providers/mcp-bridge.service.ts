...
export class McpBridgeService {
  ...
  async buildToolset(servers: RemoteMcpServer[]): Promise<BridgedToolset> {
    ...
    while (tools.length < MAX_BRIDGED_TOOLS && queues.some((queue) => queue.remaining.length)) {
      for (const queue of queues) {
        if (tools.length >= MAX_BRIDGED_TOOLS) break;
    ...
    if (total > MAX_BRIDGED_TOOLS) {
      const perApp = queues
        .map((queue) => `${queue.server.appSlug}:${queue.remaining.length} dropped`)
        .join(', ');
      this.logger.warn(
        `Connected apps expose ${total} tools; sending ${tools.length} spread across ${apps.length} apps (${perApp})`,
      );
    }
