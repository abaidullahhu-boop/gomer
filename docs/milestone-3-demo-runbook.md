...
### 1b. Disconnect both integrations in Gaspo
...
Gaspo's disconnect drops the local tokens, but **Facebook and Google still have
...
- **Meta**: facebook.com → Settings & Privacy → Settings → **Business
  Integrations** → find Gaspo → **Remove**.
- **Google**: myaccount.google.com/permissions → find the Pipedream/Gaspo entry →
  **Remove access**.
...
> member than the one connecting here, so a private connection would leave Gaspo
...
carries conversation memory of the old campaign, and Gaspo will reference things
...
| 3 | `create a paused traffic campaign called "Milestone 3 Demo" with a 5000 PKR daily budget` | Gaspo asks which account — disambiguation. |
