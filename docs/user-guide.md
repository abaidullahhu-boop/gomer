...
Gaspo is an AI coworker for your ad operations. It lives in Slack, connects to
...
- [Talking to Gaspo](#talking-to-gaspo)
...
- [Alerts](#alerts)
- [Teaching Gaspo about your business](#teaching-gaspo-about-your-business)
...
**1. Install Gaspo into Slack.** From the dashboard, choose *Add to Slack* and
approve the permissions. Gaspo will send you a short intro message. Whoever
installs it becomes the workspace admin.

**2. Connect your accounts.** Dashboard → **Integrations** → find the service →
**Connect**. Sign in to that service as you normally would; Gaspo never sees or
...
- **Team (shared)** — everyone in your Slack workspace can use it through Gaspo.
...
as **Private** and then a colleague asks Gaspo about it, Gaspo will correctly say
...
> Google's consent screen. A read-only grant looks fine until Gaspo tries to
...
**DM it.** Message Gaspo directly like a coworker. Best for anything you don't
need the rest of the team to see.

**@mention it in a channel.** Gaspo reads the thread it's mentioned in, so you
...
discussing. Starting a *new* thread gives Gaspo a clean slate, which is what you
...
Gaspo pulls live numbers from the ad accounts you've connected. It will tell you
...
Gaspo pairs ad spend with actual Stripe revenue and shows you the verified figure
...
per-campaign attribution model. Gaspo states this caveat rather than letting you
...
Gaspo can pause, resume, scale, adjust budgets and bids, and duplicate campaigns.

- *pause the campaigns with a CPA over £40*
- *increase the budget on the winning ad set by 20%*

**Changes to Meta Ads always ask first.** Gaspo describes exactly what it's about
...
Gaspo creates the sheet, writes the headers and rows, and gives you the link.
...
Gaspo confirms the schedule before creating it, and reuses the spreadsheet you
...
A rule is a standing instruction Gaspo checks on a schedule and acts on.
...
Gaspo reads back the metric, threshold, window, action, schedule, and guardrails,
...
Rules and one-off changes are different things. Asking Gaspo to pause something
...
Separate from rules, Gaspo watches your Meta ad accounts every hour and speaks up
...
## Teaching Gaspo about your business

Tell Gaspo things once and it remembers them across every future conversation.
...
| **Skills** | Install expertise packs — creative analysis, PMax audits, attribution diagnostics and similar — which Gaspo draws on when relevant |
| **Tasks** | Your scheduled prompts |
| **Automations** | What Gaspo runs unattended: rules and what they've triggered, scheduled reports, remembered facts, verified-ROAS history |
...
change these by asking Gaspo, because it confirms the details with you first —
...
Gaspo runs on credits. **100 credits = $1.**

Every new workspace starts with **$100 of free credits**.

Credits are consumed when Gaspo thinks — answering a question, running a
scheduled task, evaluating a rule. Connecting accounts, browsing the dashboard,
and reading old messages are free.

You'll get:

- a **heads-up in Gaspo's replies** once the balance is under $10
- a **clear stop** at zero: Gaspo explains it's out of credits and links you to
...
| Gaspo can't write to your spreadsheet | Google was granted read-only access | Revoke at [myaccount.google.com/permissions](https://myaccount.google.com/permissions), reconnect, grant write |
| Gaspo mentions campaigns that don't exist | You're in an old thread carrying stale context | Start a new thread |
...
| Gaspo answers about the wrong account | Multiple ad accounts connected | Name the account explicitly in your question |
| "Too many requests" | You've made a lot of requests very quickly | Wait a few minutes |

If Gaspo can't do something, it will say so plainly and tell you why. It's built
